...
class SyncDatabaseService {
  ...
  static toggleFavorite({
    ...
    return db.writeTxn(() async {
      if (await isFavorite(
        ...
        var deleteAll = db.favorites
            .filter()
            .packageEqualTo(package)
            .and()
            .urlEqualTo(url)
            .deleteAll();
        deleteFavorites(package,url);
        return deleteAll;
      } else {
  ...
  static Future<bool> isFavorite({
    ...
    await getFavorites(null,url,package);
    return (
        await db.favorites
            .filter()
            .packageEqualTo(package)
  ...
  static Future<List<Favorite>> getFavoritesByType({
    ...
    await getFavorites(type,null,null);
    if (type == null) {
  ...
  static Future<List<History>> getHistorysByType({ExtensionType? type}) async {
    await getHistory(type!=null?type.name:null);
    if (type == null) {
  ...
  static Future<History?> getHistoryByPackageAndUrl(
    ...
    History? history = await SyncServerApi.getMiruHistory(package,url);
    if(history!=null){
      var writeTxn = await db.writeTxn(() => db.historys.putByIndex(r'package&url', history));
    }
    return db.historys
        .filter()
        .packageEqualTo(package)
  ...
  static Future<Id> putHistory(History history) async {
    var writeTxn = await db.writeTxn(() => db.historys.putByIndex(r'package&url', history));
    await SyncServerApi.addHistory(history);
    return writeTxn;
  }
  // 增加收藏
  static Future<void> addFavorites(Favorite fav) async{
    await SyncServerApi.addFavorites(fav);
  }
  // 删除收藏
  static Future<void> deleteFavorites(String package, String url) async{
    await SyncServerApi.deleteFavoritesSync(package,url);
  }

  // 删除历史
  static Future<void> deleteHistoryByPackageAndUrl(
      String package, String url) async {
    var writeTxn = await db.writeTxn(
          () => db.historys
          .filter()
          .packageEqualTo(package)
          .urlEqualTo(url)
    ...
    await SyncServerApi.deleteHistoryByPackageAndUrlSync(package,url);
  }

  // 删除全部历史
  static Future<void> deleteAllHistory() async {
    await db.writeTxn(() => db.historys.where().deleteAll());
    await SyncServerApi.deleteHistoryByPackageAndUrlSync(null,null);
  }

  //获取服务器历史记录
  static Future<void> getHistory(String? type) async {
    try{
      List<History> history = await SyncServerApi.getHistory(type);
      for (History value in history) {
        await db.writeTxn(() => db.historys.putByIndex(r'package&url', value));
      }
    }catch(e){
      log("获取服务器历史记录失败",error: e);
    }
  }


  //获取服务器收藏
  static Future<void> getFavorites(ExtensionType? type,String? url,String? package) async {
    try{
      List<Favorite> favorites = await SyncServerApi.getFavorites(type!=null?type.name:null,url,package);
      for (Favorite value in favorites) {
        var writeTxn = await db.writeTxn(() => db.favorites.putByIndex(r'package_url', value));
      }
    }catch(e){
      log("获取服务器收藏失败",error: e);
    }
  }
